                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div>
                                <a href="{{ route('admin.roles.user.create') }}" class="btn btn-primary">+ Add
                                    New</a>
                            </div>
                        </div>
                                    ...
                                    @foreach ($admins as $admin)
                                        <tr>
                                            ...
                                            <td data-label="Permissions" class="text-center">
                                                @if (strtolower($admin->name) == 'super admin')
                                                    <span
                                                        class="badge bg-blue text-blue-fg">{{ __('All permissions') }}</span>
                                                @else
                                                    {{ $admin->permissons_count }}
                                                @endif
                                            </td>
                                            <td data-label="Role" class="text-center">
                                                {!! count($admin->getRoleNames()) > 0
                                                    ? '<span class="badge badge-outline text-indigo">' . $admin->getRoleNames()->implode(', ') . '</span>'
                                                    : '<span class="badge badge-outline text-yellow">Role Not Added</span>' !!}
                                            </td>
                                            <td>
                                                @if (strtolower($admin->name) != 'super admin')
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu dropdown-menu-end" style="">
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.roles.user.edit', ['user' => $admin->id]) }}">
                                                                    Edit </a>

                                                                <a href="#"
                                                                    class="dropdown-item show-alert-delete-box"
                                                                    data-form-id="delete-form-{{ $admin->id }}">
                                                                    Delete
                                                                </a>

                                                                <form id="delete-form-{{ $admin->id }}"
                                                                    class="delete-form d-none" method="POST"
                                                                    action="{{ route('admin.roles.user.destroy', ['user' => $admin->id]) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                </form>
                                                            </div>
